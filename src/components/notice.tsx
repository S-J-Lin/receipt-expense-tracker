...
export function Notice({ success, error, warning }: { success?: string; error?: string; warning?: string }) {
  ...
  ) : warning === "alias-failed" ? <p className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm font-medium text-amber-900" role="status">消費明細已成功儲存，但部分商品別名未能儲存，請稍後重試。</p>
    : warning === "alias-saved" ? <p className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm font-medium text-emerald-800" role="status">商品名稱對應已記住。</p> : null;
