...
export default async function ExpenseDetailPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return (
    <main className="flex-1 px-4 py-6 sm:px-6"><div className="mx-auto max-w-2xl space-y-4">
      ...
      <Notice error={single(query.error)} success={single(query.success)} />
      <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-8">
        ...
        {expense.receipt_image_url && <div className="mt-5"><p className="mb-2 font-medium">收據圖片</p><img alt={`${expense.merchant} 收據`} className="max-h-[32rem] w-full rounded-2xl object-contain" src={expense.receipt_image_url} /></div>}
