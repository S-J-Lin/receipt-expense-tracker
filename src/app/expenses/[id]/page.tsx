  ...
  return (
    <main className="flex-1 px-4 py-6 sm:px-6"><div className="mx-auto max-w-2xl space-y-4">
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-8">
        ...
        {expense.expense_items.length > 0 && <div className="mt-6"><h2 className="text-lg font-bold">商品明細</h2><div className="mt-2 divide-y divide-slate-100 rounded-2xl border border-slate-200">{expense.expense_items.map((item) => <div className="grid grid-cols-[1fr_auto] gap-3 p-3" key={item.id}><div><p className="font-medium">{item.name_normalized || item.name_original}</p>{item.english_name && <p className="text-sm text-slate-600">{item.english_name}</p>}{item.name_normalized && item.name_original && <p className="text-xs text-slate-500">{item.name_original}</p>}<p className="mt-1 text-sm text-slate-500">{item.brand} · {item.product_group ? `${item.product_group} · ` : ""}數量 {item.quantity}{item.unit_quantity ? ` · ${item.unit_quantity} ${item.unit ?? ""}` : ""} · {item.category}{item.confidence != null ? ` · 信心 ${Math.round(item.confidence * 100)}%` : ""}</p>{item.notes && <p className="mt-1 text-sm text-slate-600">{item.notes}</p>}</div><p className="font-semibold">{formatExpenseAmount(item.amount, expense.currency)}</p></div>)}</div></div>}
