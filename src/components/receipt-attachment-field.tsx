...
export function ReceiptAttachmentField({ currentKind, currentPath, currentUrl, expenseId }: Props) {
  const uploadHref = expenseId ? `/receipts/upload?expenseId=${expenseId}` : "/receipts/upload";
  return (
    <section className="space-y-3 rounded-2xl border border-slate-200 bg-slate-50 p-4">
      ...
      {currentPath && <input name="receipt_image_path" type="hidden" value={currentPath} />}
