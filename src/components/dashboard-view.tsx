...
function Donut({ summary, currency }: { summary: CurrencyPeriodSummary; currency: string }) {
  ...
  return <section className={card}>
    <div><p className="text-sm font-semibold text-slate-500">本月支出分布 · {currency}</p><p className="dashboard-amount mt-2 font-bold">{formatMoneyFromCents(summary.totalCents, currency)}</p><p className="mt-2 text-xs text-slate-500">包含房租與所有已發生支出</p></div>
...
export function DashboardView({ month, mode, periods, monthExpenses, currentWeek, previousWeek, currentMonth, previousMonth, actualMonth, recurring, recurringError }: {
  ...
  return <>
    <section aria-labelledby="daily-analysis-title" className="flex min-w-0 flex-wrap items-center justify-between gap-3"><div><h2 className="text-xl font-bold" id="daily-analysis-title">日常消費分析</h2><p className="mt-1 text-sm text-slate-500">週、月比較與預估採用日常消費口徑</p></div><span className="shrink-0 rounded-full border border-indigo-200 bg-indigo-50 px-3 py-1 text-sm font-semibold text-indigo-700">不含房租</span></section>
    ...
    {currencies.length > 0 && <section aria-label="快速指標" className="grid min-w-0 grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">{currencies.map((currency) => {
      ...
      const previous = previousMonth.get(currency); const actual = actualMonth.get(currency);
      ...
      return <div className="contents" key={currency}>
        <article className={card}><p className="text-sm font-semibold text-slate-500">每日平均 · {currency}</p><p className="metric-amount mt-3 font-bold">{formatMoneyFromCents(daily, currency)}<span className="ml-1 text-sm font-medium text-slate-500">/ 日</span></p><p className="mt-2 text-xs text-slate-500">不含房租 · 比較期 {formatMoneyFromCents(previousDaily, currency)} / 日</p></article>
        <article className={card}><p className="text-sm font-semibold text-slate-500">月底日常支出預估 · {currency}</p>{summary.totalCents === 0 ? <p className="mt-3 text-sm text-slate-500">本月尚無日常消費資料</p> : <><p className="metric-amount mt-3 font-bold">約 {formatMoneyFromCents(projectMonthCents(summary.totalCents, monthDaysElapsed, daysInIsoMonth(month)), currency)}</p><p className="mt-2 text-xs text-slate-500">依目前非房租支出速度估算{monthDaysElapsed <= 3 ? "；月初波動較大" : ""}</p></>}</article>
        <article className={card}><p className="text-sm font-semibold text-slate-500">本月交易 · {currency}</p><p className="metric-amount mt-3 font-bold">{actual?.count ?? 0} 筆</p><p className="mt-2 text-xs text-slate-500">包含所有實際支出</p></article>
        <article className={card}><p className="text-sm font-semibold text-slate-500">平均每筆 · {currency}</p><p className="metric-amount mt-3 font-bold">{formatMoneyFromCents(averageCents(summary.totalCents, summary.count), currency)}</p><p className="mt-2 text-xs text-slate-500">不含房租</p></article>
      </div>;
    })}</section>}
